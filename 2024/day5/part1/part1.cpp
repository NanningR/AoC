...
namespace
{
    ...
    void countValidRows(vector<string> const &input, int &answer)
    {
        ...
        for (auto const &el : cases)
        {
            ...
            // This brute force is probably not the optimal solution..
            for (int i = 0; i < numbers.size(); i++)
            {
                int current = numbers[i];

                vector<int> before(numbers.begin(), numbers.begin() + i);
                vector<int> after(numbers.begin() + (i + 1), numbers.end());
                ...
                for (auto const &b : before)
                {
                    for (auto const &order : orders)
                    {
                        ...
                        if (current == orderPair.first && b == orderPair.second)
                            isValid = false;
                    }
                }
                ...
                for (auto const &a : after)
                {
                    ...
                    {
                        auto orderPair = getFirstAndSecond(order);
                        if (current == orderPair.second && a == orderPair.first)
                            isValid = false;
                    }
                }
            }
